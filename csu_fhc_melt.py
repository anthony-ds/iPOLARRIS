...
Thompson, E. J., Rutledge, S. A., Dolan, B., Chandrasekar, V., & Cheong,
B. L. (2014). A dual-polarization radar hydrometeor classification algorithm
for winter precipitation. Journal of Atmospheric and Oceanic Technology,
31(7), 1457-1481.
...
from beta_functions import CSV_DIR


def melting_layer(dz=None, zdr=None, ldr=None, kdp=None, rho=None, sn=None,
    ...
    if sn is not None:
        ...
        if verbose:
            print('Wet snow detection will be improved',
                  'by including the SN field.')

    # Now for the ML algorithm Rho is the most important variable.
    # So if the rho was not valid, and where rho is less than the threshold.
    ...
    # Now use the returned Wet snow field to look at melting layer statistics
    # and try to refine the location of the ML. There are 2 different methods
    # based on if the scan is an rhi or ppi.
    if heights is None:
...
def get_ml_rhi(fh, dz, height, expected_ML, verbose=False):
    # This will return two things. The first is an array the same size as
    # FH data identifying the melting region, the cold and warm regions
    # meltlev = 0   Warm
    ...
    if expected_ML is not None:
        if verbose:
            print('Using an expected ML',
                  'of {m} to narrow the results'.format(m=expected_ML))
        wh_ht = np.ma.logical_and(height <= expected_ML+1.5,
                                  height >= expected_ML-1.5)
    ...
    # you are going to have some error messages if it doesn't find a
    # melting layer - it needs an array to do these statistics
    # this algorithm is designed for stratiform precipitation, so if it
    # doesn't have a ML you really should be using a different algorithm
    ...
    if len(ML_z) > 10:
        ...
        print('not enough points for ML detection.',
              'Using expected: {m}'.format(m=expected_ML))
...
def get_ml_ppi(fh, dz, height, expected_ML,
               nsect=36, verbose=False, azimuths=None):
    ...
    if azimuths is not None:
        ...
        meltlev = np.zeros_like(fh).astype(float) + 1.0
        ...
        for i, a in enumerate(az_sector[:-1]):
            ...
            if verbose:
                print('Melting layer for {a}-{b}: {m}'.format(
                    a=a, b=az_sector[i+1], m=ML_medianZ[i]))
