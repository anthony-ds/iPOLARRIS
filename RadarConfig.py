...
class RadarConfig(object):
    ...
    def __init__(self, dz='DZ', zdr='DR', kdp='KD', ldr='LH', rho='RH', hid = 'HID',conv='Con',temp='T', x='x', y='y', z='z', u='u', v='v',rr='RR', w='w',qc='qc',qr='qr',qg='qg',qh='qh',qi='qi',vr='VR',exper = 'Case',rtype='obs',rsrc='nexrad',band = 'C',lat_0 = 0,lon_0=90.0,lat_r=None,lon_r=None,lat=None,lon=None,tm = None,radar_name = None,color_blind = False,dd_on=False,rr_on=False,hid_on=True,hid_cats = 'summer'):
        ...
        self.q_vars = [self.qc_name,self.qr_name,self.qg_name,self.qi_name,self.rr_name if self.rr_on else None,self.hid_name if self.hid_on else None] #Mixing ratios from model to plot.
        ...
        self.names = {self.dz_name: 'Z', self.zdr_name: 'Z$_{DR}$', self.kdp_name: 'K$_{dp}$', self.ldr_name: 'LDR', self.rho_name: r'$\rho_{hv}$', self.hid_name: '',self.w_name:'',self.vr_name:'V$_r$',self.cs_name:'',self.rr_name:'RR',self.temp_name:'T',self.qc_name:'Q$_{C}$',self.qr_name:'Q$_{R}$',self.qg_name:'Q$_{G}$',self.qh_name:'Q$_{H}$',self.qi_name:'Q$_{I}$'}
        self.names_uc = {self.dz_name: 'REF', self.zdr_name: 'ZDR', self.kdp_name: 'KDP', self.ldr_name: 'LDR', self.rho_name: 'RHO', self.hid_name: 'HID', self.u_name:'U', self.w_name:'W',self.vr_name:'VRAD',self.cs_name:'',self.rr_name:'RR',self.temp_name:'T',self.qc_name:'QC',self.qr_name:'QR',self.qg_name:'QG',self.qh_name:'QH',self.qi_name:'QI'}
