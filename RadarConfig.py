...
class RadarConfig(object):
    ...
    def __init__(self, dz='DZ', zdr='DR', kdp='KD', ldr='LH', rho='RH', hid = 'HID',conv='Con',
        ...
        self.lims = {dz: [0,80], zdr: [-1, 3], kdp: [-0.5, 3], ldr: [-35, -20], rho: [0.95, 1.00], hid: [0,len(self.species)+1],w:[-2,2],vr:[-25,25],self.cs_name:[0,4],self.rr_name:[0,30],self.temp_name:[-30,30]}
        self.cfbins = {dz: np.arange(-10,60.1,1), zdr: np.arange(-2,6.01,0.05), kdp: np.arange(-2,2.01,0.05), rho: np.arange(0.5,1.01,0.02), hid: '' , w: np.arange(-6,6.1,0.5), self.temp_name: np.arange(20,-60.1,-5)} 
        ...
        self.ticklabels = {dz: np.arange(0, 90, 10), zdr: np.arange(-1, 4, 1), kdp: np.arange(-0.5, 4.5, 1), 
                ldr: np.arange(-35, -15, 5), rho: np.arange(0.95, 1.01, 0.005), hid: np.append('', self.species),w:np.arange(-2,2.1,0.1),vr:np.arange(-25,30.0,5.0),
